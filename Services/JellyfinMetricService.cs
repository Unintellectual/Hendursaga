...
public class JellyfinMetricsService : BackgroundService
{
    ...
    private readonly Gauge _activeUsers = Metrics.CreateGauge("jellyfin_active_users", "Number of active users");
    private readonly Gauge _streamingSessions = Metrics.CreateGauge("jellyfin_streaming_sessions", "Number of active streaming sessions");

    public JellyfinMetricsService(HttpClient httpClient, ILogger<JellyfinMetricsService> logger)
    {
    ...
    private async Task CollectMetricsAsync()
    {
        try
        {
            ...
            _activeUsers.Set(activeUsers);
            _streamingSessions.Set(sessionCount);
